...
test('project reports keep their data and downloads on their own pages', async ({ page }) => {
  const errors: string[] = []
  ...
  await expect(drugData.getByRole('combobox', { name: 'Dataset', exact: true }).locator('option')).toHaveCount(53)
  await expect(drugData.locator('caption')).toHaveText('metrics')
  await drugData.getByRole('combobox', { name: 'Evaluation split', exact: true }).selectOption('cold_drug')
  await expect(drugData.locator('tbody tr')).toHaveCount(8)
  await expect(drugData.getByRole('link', { name: 'Original CSV' })).toHaveAttribute('href', /metrics.csv$/)
  ...
  await politicsData.getByRole('button', { name: 'All decision points' }).click()
