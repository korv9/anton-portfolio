...
function Reader({ card }: { card: Card }) {
  ...
  const speeches = (data?.data ?? []).filter(
    (s) =>
      s.speech_number >= (card.first ?? -1) &&
      s.speech_number <= (card.last ?? Infinity),
  )
  const index = speeches.findIndex((s) => s.speech_id === selected)
  const speech = speeches[index]
  return (
    <article
      className="speech-reader"
      aria-label={t('Selected speech')}
      ref={reader}
      tabIndex={-1}
    >
      <p className="eyebrow">
      ...
      {!data && !error && <p role="status">{t('Loading the full debate…')}</p>}
      {speech && (
        <>
          <h4>
          ...
          <div className="table-pagination">
            <button
              disabled={index <= 0}
              onClick={() => setSelected(speeches[index - 1].speech_id)}
            >
              {t('Previous speech')}
            </button>
            <button
          ...
          <p className="source-text" lang="sv">
            {speech.speech_text}
          </p>
          ...
          <details>
            <summary>
              {t('Other speakers in this discussion (')}
              {speeches.length})
            </summary>
...
export default function SpeechBrowser() {
  const manifest = useData<{ session: string; count: number; path: string }[]>(
    'index.json',
    'discovery/',
  )
  ...
  const matches = (data ?? []).filter(
    (s) =>
      ...
      `${s.speaker} ${s.title} ${s.excerpt}`
        .toLocaleLowerCase()
        .includes(query.toLocaleLowerCase()),
  )
  return (
    <section className="report speech-browser" id="data-explorer">
      <p className="eyebrow">{t('Explore Swedish politics')}</p>
      <h2>{t('What did they actually say?')}</h2>
      ...
      <div className="politics-controls" id="speech-search">
        <label>
          ...
          <input
            ...
            placeholder={t('Name, debate title or opening words')}
          />
        </label>
        <label>
          {t('Party')}
          ...
          >
            ...
            {[...new Set([...PARTIES, ...(data ?? []).map((s) => s.party)])]
              .sort()
              .map((p) => (
      ...
      {(error || manifest.error) && (
        <p role="alert">{error || manifest.error}</p>
      )}
      {!data && !error && !manifest.error && (
        <p role="status">{t('Loading speeches…')}</p>
      )}
      {data && (
        <>
          <p aria-live="polite">
          ...
          <div className="speech-browse-layout">
            <div>
              ...
              {matches.length > limit && (
                ...
                >
                  {t('Show more speeches')}
                </button>
              )}
            </div>
            {selected ? (
              <Reader
                key={selected.kind + selected.speech_id}
                card={selected}
              />
            ) : (
              <aside className="speech-reader">
                <h3>{t('Start with a person or a question.')}</h3>
                <p>
                  {t(
