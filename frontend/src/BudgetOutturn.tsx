...
type Row = {
  ...
  approved_budget_msek: number
  amendments_msek: number
  outturn_msek: number
  source_url: string
}
...
const signed = (value: number) =>
  `${value > 0 ? '+' : value < 0 ? '−' : ''}${bn(Math.abs(value))}`
...
export default function BudgetOutturn() {
  ...
  const years = [...new Set(rows.map((row) => row.budget_year))].sort(
    (a, b) => b - a,
  )
  const annual = rows
    .filter((row) => row.budget_year === year)
  ...
  return (
    <section className="report outturn-report" id="budget-outturn">
      ...
      {error && (
        <p role="alert">{t('The annual accounts could not be loaded.')}</p>
      )}
      {!rows.length && !error && (
        <p role="status">{t('Loading annual accounts…')}</p>
      )}
      {rows.length > 0 && (
        <>
          ...
          <div className="budget-ledger-summary">
            <div>
              ...
              <span>{t('Approved budget · 27 areas')}</span>
            </div>
            <div>
              <strong>{bn(actual)}</strong>
              <span>{t('Recorded expenditure · 27 areas')}</span>
            </div>
            <div>
              <strong>{signed(actual - approved)}</strong>
              <span>{t('Outturn minus approved budget')}</span>
            </div>
            <div>
              <strong>
          ...
          <div className="budget-ledger-layout">
            ...
            <aside className="budget-ledger-detail">
              {focused && (
                <>
                  <p className="eyebrow">
                    {focused.expenditure_area}.{' '}
                    {nameOf(focused.expenditure_area)}
                  </p>
                  <strong>{bn(focused.outturn_msek)}</strong>
                  <p>
                    {t('Recorded expenditure · approved budget ')}
                    {bn(focused.approved_budget_msek)} {t('· difference ')}
                    {signed(
                      focused.outturn_msek - focused.approved_budget_msek,
                    )}
                    .
                  </p>
                  <p>
                    {t('Reported amendments: ')}
                    {signed(focused.amendments_msek)}
                    {t(
          ...
          <details className="budget-ledger-all">
            <summary>
              {t('Read all 27 areas for ')}
              {year}
            </summary>
            <div
              className="coverage-scroll"
              tabIndex={0}
              aria-label={t('Annual account table, scroll horizontally')}
            >
              <table>
                <thead>
                  <tr>
                    <th scope="col">{t('Area')}</th>
                    <th scope="col">{t('Approved budget')}</th>
                    <th scope="col">{t('Recorded expenditure')}</th>
                    <th scope="col">{t('Difference')}</th>
                    <th scope="col">{t('Reported amendments')}</th>
                  </tr>
                </thead>
                <tbody>
                  {annual.map((row) => (
                    <tr key={row.expenditure_area}>
                      <th scope="row">
                        {row.expenditure_area}. {nameOf(row.expenditure_area)}
                      </th>
                      <td>{bn(row.approved_budget_msek)}</td>
                      <td>{bn(row.outturn_msek)}</td>
                      <td>
                        {signed(row.outturn_msek - row.approved_budget_msek)}
                      </td>
                      <td>{signed(row.amendments_msek)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
