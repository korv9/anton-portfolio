...
export default function ProjectDataDisclosure({ title, sectionId, initialDataset }: { title: string; sectionId: string; initialDataset: string }) {
  const [expanded, setExpanded] = useState(() => window.location.hash === '#' + sectionId)
  useEffect(() => {
    const followLink = () => { if (window.location.hash === '#' + sectionId) setExpanded(true) }
