...
-- weight, correct for estimates within one country; `anweight` is missing in rounds 2-3
-- and differs from it only by a constant within a country.
...
with respondents as (
    select
        ...
        * exclude (study, datafile, essround, country_code, respondent_id,
                   post_stratification_weight, design_weight, analysis_weight,
    ...
    on columns(* exclude (period_key, essround, datafile, survey_mode, country_code,
                          respondent_id, weight, sex_key, age_group_key, nuts2_code))
