...
test('home introduces Anton and routes to each project', async ({ page, isMobile }) => {
  const errors: string[] = []
  ...
  await page.getByRole('navigation', { name: 'Main navigation' }).getByRole('link', { name: 'Politics' }).click()
  await expect(page.getByRole('heading', { level: 1 })).toContainText('Swedish politics')
...
test('language map and job chart retain useful controls', async ({ page }) => {
  ...
  await expect(page.locator('.umap-guide')).toContainText('not agreement or a political position')
  await page.goto('/#job-market')
...
test('navigation, responsive layout and accessibility', async ({ page }) => {
  ...
  for (const route of ['/', '/#politics', '/#budget-comparison', '/#drugcomb']) {
...
test('budget proposals and annual outcomes are separate navigable reports', async ({ page }) => {
  ...
  await budget.getByRole('combobox', { name: 'Budget year', exact: true }).selectOption('2017/18')
  await expect(budget.locator('.budget-ledger-summary')).toContainText('21/27')
  await expect(budget.locator('.budget-ledger-summary')).toContainText('Incomplete')
  await budget.getByRole('combobox', { name: 'Budget year', exact: true }).selectOption('2025/26')
  await budget.getByRole('combobox', { name: 'Proposal', exact: true }).selectOption('V')
  await expect(budget.locator('.budget-ledger-detail')).toContainText('relative to government')
  ...
  await expect(page.locator('#budget-outturn .budget-ledger-all summary')).toContainText('2024')
})
