...
function App() {
  ...
  return <>
    ...
    <header className="site-header"><a className="wordmark" href="#start" aria-label="Anton Ernstsson, home"><span>AE</span><strong>Anton Ernstsson</strong></a><button className="menu-button" onClick={() => setMenuOpen(!menuOpen)} aria-expanded={menuOpen} aria-controls="site-nav">{menuOpen ? 'Close' : 'Menu'}</button><nav id="site-nav" className={menuOpen ? 'nav open' : 'nav'} aria-label="Main navigation"><a href="#start" onClick={() => setMenuOpen(false)}>About</a><a href="#projects" onClick={() => setMenuOpen(false)}>Projects</a><a href="#politics" onClick={() => setMenuOpen(false)}>Politics</a><a href="#job-market" onClick={() => setMenuOpen(false)}>Job market</a><a href="#drugcomb" onClick={() => setMenuOpen(false)}>DrugComb</a><a className="nav-cv" href="/Anton_Ernstsson_CV_Data_Engineer.pdf" download>Download CV</a></nav></header>
    <main id="main">
      {page === 'home' && <>
<section className="intro profile-home" id="start"><div className="intro-copy" id="about"><p className="eyebrow">Anton Ernstsson · Stockholm</p><h1>Data Engineer<br />& Analytics Engineer.</h1><p className="lede">Hi, I'm Anton. I enjoy making complex information easier to understand — from building reliable data pipelines to exploring questions through reports and applied AI. I'm a junior data professional with experience from Fora and Avtalat, and this is a collection of what I've worked on and what I'm curious about.</p><div className="intro-links"><a href="#projects">View projects <span>↓</span></a><a href="mailto:anton.ernstson@gmail.com">Email me</a><a href="/Anton_Ernstsson_CV_Data_Engineer.pdf" download>Download CV</a></div></div>
      <aside className="profile-board" aria-label="Profile overview">
        <section><p className="eyebrow">Experience</p><div className="profile-rows"><div><time>2026</time><span><strong>Avtalat</strong><small>Analytics Engineer · LIA internship</small></span></div><div><time>2025–26</time><span><strong>Fora</strong><small>Data Engineer · LIA internship</small></span></div><div><time>2024–26</time><span><strong>JENSEN</strong><small>AI Developer programme</small></span></div></div></section>
      ...
      {page === 'politics' && <div className="project-page politics-page"><div className="page-lead" id="politics-page"><p className="eyebrow">Personal research · Swedish politics</p><h1>Swedish politics, in the records.</h1><p>Speeches, proposed spending and formal decisions in one place. I built this to make it easier to follow what politicians actually do and check the original sources.</p></div>
        <nav className="page-tabs" aria-label="Politics report views"><a href="#politics" aria-current={politicsView === 'overview' ? 'page' : undefined}>Decisions</a><a href="#data-explorer" aria-current={politicsView === 'speeches' ? 'page' : undefined}>Speeches</a><a href="#budget-comparison" aria-current={politicsView === 'budgets' ? 'page' : undefined}>Budgets</a><a href="#debates" aria-current={politicsView === 'language' ? 'page' : undefined}>Language map</a><a href="#raw-data" aria-current={politicsView === 'data' ? 'page' : undefined}>Data & methods</a></nav>
        ...
        <div className="viz-shell"><div className="viz-toolbar"><div><span className="control-label">Parliamentary session</span><div className="segmented">{sessions.map((item) => <button key={item} className={session === item ? 'active' : ''} onClick={() => { setLoading(true); setSession(item); setParty('All') }}>{item.replace('-', '/')}</button>)}</div></div><label><span className="control-label">Party</span><select value={party} onChange={(event) => { const nextParty = event.target.value; setParty(nextParty); setSelectedPoint(umap.find((point) => nextParty === 'All' || point.party === nextParty) ?? null) }}>{parties.map((item) => <option key={item}>{item}</option>)}</select></label></div>
          <div className="umap-guide"><div><strong>How to read the map</strong><span>Each dot is one sampled speech segment. Distance shows approximate language similarity, not agreement or a political position. Cluster names are provisional machine labels; I plan to retrain this model.</span></div><dl><div><dt>Shown</dt><dd>{formatNumber(visiblePoints.length)}</dd></div><div><dt>Sample assigned a cluster</dt><dd>{groupedShare.toFixed(0)}%</dd></div><div><dt>Full session</dt><dd>{formatNumber(sessionPopulation)}</dd></div></dl></div>
      ...
      {page === 'jobs' && <div className="project-page"><div className="page-lead"><p className="eyebrow">Swedish Job Market Analytics</p><h1>Swedish job market.</h1><p>Historical job-ad data, with clear definitions and a view of how software and data roles changed.</p></div><div className="reports"><article className="report" id="job-market"><ReportHeader number="02" eyebrow="Labour market & technology" title="What is happening to tech jobs?" intro="A compact view of ad volume, junior openings and technologies mentioned in Swedish job ads." source="JobTech Historical Ads" period="2022–2025" unit="Unique ad IDs" /><div className="kpis jobs-kpis"><div><strong>{jobKpis ? formatNumber(jobKpis.ads_total) : '—'}</strong><span>ads in the sample</span></div><div><strong>{jobKpis ? formatNumber(jobKpis.employers_unique) : '—'}</strong><span>unique employers</span></div><div><strong>{jobKpis ? formatSignedPercent(jobKpis.software_change_2022_2025_pct) : '—'}</strong><span>developer ads, 2022–25</span></div><div><strong>{jobKpis ? `${jobKpis.junior_share_pct.toFixed(1)}%` : '—'}</strong><span>junior share</span></div></div>
        ...
        <details className="method"><summary>Definitions & limitations</summary><div><p>Documented text rules define role families and detect technology mentions. A mention may be optional or negated.</p><p>An ad is not a hire. The archive may not cover every Swedish vacancy, and title-based seniority is an approximation.</p><a href="https://github.com/korv9/swedish-job-market-analytics" target="_blank" rel="noreferrer">Code and methodology on GitHub ↗</a></div></details></article></div><ProjectDataDisclosure title="Job ad tables and definitions" initialDataset="fact_job_month_role" sectionId="job-data" /></div>}
      {page === 'drugcomb' && <div className="project-page"><div className="page-lead"><p className="eyebrow">DrugComb Synergy Prediction</p><h1>DrugComb Synergy Prediction.</h1><p>Experimental results from cleaned measurements through evaluation on unfamiliar pairs, drugs and cell lines.</p></div><div className="reports"><DrugCombReport /><ProjectDataDisclosure title="Result tables and downloads" initialDataset="drugcomb_metrics" sectionId="drugcomb-data" /></div></div>}
      {page === 'allegoria' && <div className="project-page"><div className="page-lead"><p className="eyebrow">Allegoria · work in progress</p><h1>Allegoria / RFC drift.</h1><p>A compact experiment with real requirement profiles and a clearly labelled synthetic direction example.</p></div><div className="reports"><RfcReport /></div></div>}
      {page === 'thesis' && <div className="project-page"><div className="page-lead"><p className="eyebrow">Degree project · Fora</p><h1>Finding useful review candidates in incident data.</h1><p>My principal case study in data quality, privacy-aware NLP and clustering. Internal source records are not published.</p></div><div className="featured-projects"><div className="project-grid"><article className="thesis-card" id="thesis"><div className="project-card-head"><span>Degree project · Fora · 2026</span><strong>Primary case study</strong></div><h3>Finding review candidates in incident data</h3><p>Built a privacy-aware Azure Databricks workflow for data quality assessment and NLP clustering. The goal was to surface groups of similar incidents for manual review, without presenting clusters as proven root causes.</p><div className="thesis-kpis"><div><strong>16,811</strong><span>anonymised incidents</span></div><div><strong>121</strong><span>clusters found</span></div><div><strong>72</strong><span>without an existing problem link</span></div><div><strong>0.706</strong><span>best reported silhouette</span></div></div><div className="thesis-findings"><div><span>01</span><p><strong>Data quality shaped the pipeline.</strong> Selected ISO/IEC 25012 dimensions were assessed before the modelling stage.</p></div><div><span>02</span><p><strong>HDBSCAN produced the stronger internal separation.</strong> The reported silhouette was 0.706, compared with 0.534 for KMeans.</p></div><div><span>03</span><p><strong>72 clusters became review candidates.</strong> They lacked an existing problem link, but require domain validation before any root-cause claim.</p></div></div><details><summary>Pipeline & limitations</summary><p>Presidio for PII, multilingual sentence embeddings, UMAP to 10 dimensions, HDBSCAN and MLflow. Internal clustering metrics do not replace business validation, and no internal incident text or employer raw data is published here.</p></details></article></div></div></div>}
      {page === 'homie' && <div className="project-page"><div className="page-lead"><p className="eyebrow">Homie API · work in progress</p><h1>From household events to understandable analytics.</h1><p>The API contract and data design are here for inspection. Several endpoints remain documented stubs.</p></div><div className="reports"><HomieProject /></div></div>}
    </main>
    <footer><span>Anton Ernstsson · Stockholm</span><span>Explore the projects · <a href="#projects">Back to all projects</a></span></footer>
  </>
}
