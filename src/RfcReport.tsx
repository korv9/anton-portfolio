        ...
        <>
          <div className="politics-grid">
            <section className="politics-card">
              ...
              <p>
                {t("RFC 2965 (2000) → RFC 6265 (2011). Share of statements extracted\n                by the heuristic reader, grouped by their first requirement\n                keyword.\n              ")}</p>
              {data.versions.map((v) => (
                <div className="rfc-version" key={v.rfc}>
                  <h4>
                    {t("RFC ")}{v.rfc}{' '}
                    <small>{v.requirements} {t("extracted statements")}</small>
                  </h4>
                  {[
              ...
              <p className="evidence-note">
                {t("A different document-wide profile is not proof that the protocol\n                became looser. These documents differ in content, scope and\n                wording.\n              ")}</p>
            </section>
            <section className="politics-card">
              <h3>{t("02 / A change the engine can explain")}</h3>
              <p>
                {t("Synthetic example · the actor and action stay identical. Only\n                obligation strength changes.\n              ")}</p>
              <label>
              ...
              <div className="rfc-example">
                <p>
                  {t("Before: The client")}{' '}
                  <strong>{scenario === 1 ? 'SHOULD' : 'MUST'}</strong> {t("validate\n                  the response.\n                ")}</p>
                <p>
                  {t("After: The client")}{' '}
                  <strong>{scenario === 0 ? 'SHOULD' : 'MUST'}</strong> {t("validate\n                  the response.\n                ")}</p>
              </div>
